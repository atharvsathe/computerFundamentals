...
#define THRESH 18   // threshold value for better fit algorithm
...
// mask to extrtact previous allocated bit from header
...
bool mm_init(void) {
  ...
  start[0] = pack(prev_alloc_mask, true); // Heap prologue (block footer)
  start[1] = pack(prev_alloc_mask, true); // Heap epilogue (block header)
...
void *malloc(size_t size) {
  ...
  int abit = (block->header) & (prev_alloc_mask);
  size_t block_size = get_size(block);
  write_header(block, block_size + abit, true);
...
static block_t *extend_heap(size_t size) {
  void *bp;
  void *end = ((char *)mem_heap_hi()) - 7;
  long mask = *((long *)end);
  long abit = (prev_alloc_mask) & mask;
...
static block_t *coalesce_block(block_t *block) {
  ...
  else if (!prev_alloc && next_alloc) // Case 3
  {
    ...
    write_header(block_prev, size + prev_alloc_mask, false);
    block = block_prev;
    insert(block);
  }
...
static void split_block(block_t *block, size_t asize) {
  ...
  {
    write_header(block, asize + abit, true);

    block_next = find_next(block);
    write_header(block_next, block_size - asize + prev_alloc_mask, false);
...
bool mm_checkheap(int line) {
  ...
  if (*((word_t *)mem_heap_hi() - 7) != (pack(0, true) & ~prev_alloc_mask)) {
  ...
  for (block = head; get_size(block) > 0; block = find_next(block)) {
    ...
    //check previous allocated bit
    if (get_alloc(find_next(block)) != (bool)(block->header & prev_alloc_mask)) {
      printf("error: %p previous allocated bit discrepancy\n", block);
      return false;
    }
  ...
  for (unsigned list_index = 0; list_index < LISTSIZE; list_index++) {
    ...
    for (block = heap_start[list_index]; block != NULL; block = NEXTBLOCK) {
      ...
      if (list_index == 0 && get_size(block) > (1 << 6)) {
        ...
        printf("error: %p incorrect list\n", block);
        return false;
      } else if (get_size(block) < (1 << (list_index + 6)) ||
                 get_size(block) >= (1 << (list_index + 7))) {
        printf("error: %p incorrect list\n", block);
        return false;
      }
    }
